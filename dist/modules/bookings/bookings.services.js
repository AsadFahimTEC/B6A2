...
const createBookings = async (customerId, vehicleId, start, end) => {
    const vehicle = await pool.query(`SELECT vehicle_name, daily_rent_price from vehicles WHERE id=$1 AND availability_status='available'`, [vehicleId]);
    ...
    const booking = await pool.query(`INSERT into bookings (customer_id, vehicle_id, rent_start_date, rent_end_date, total_price, status) 
        VALUES($1, $2, $3, $4, $5, 'active') RETURNING id, customer_id, vehicle_id, rent_start_date, rent_end_date, total_price, status `, [customerId, vehicleId, start, end, totalPrice]);
    await pool.query(`UPDATE vehicles SET availability_status='booked' WHERE id=$1 `, [vehicleId]);
    return {
        ...booking.rows[0],
        vehicle: {
            vehicle_name,
...
const getBookings = async (role, userId) => {
    if (role === "admin") {
...
const updateBookings = async (bookingId, status, role, userId) => {
